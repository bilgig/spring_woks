...
*/
	...
	Scanner scanner=new Scanner(System.in);
		...
		private void addALessonToStudent() {
			...
			System.out.println("");
			System.out.println("--Lesson Name--                PassedNote");
			...
			System.out.println("Atanacak Ders adı giriniz:  ");
			String lessonName = scanner.next();
			System.out.println();
			//girilen student name den öğrenci nosu bulunuyor
			Long studentNo = studentRepository.findByFirstName(studentName)
					.orElseThrow(() -> new RuntimeException("no student"))
					.getStudentNo();
			//Girilen ders adından dersin idsi bulunmak için önce enuma dersin adı veriyor.
			LessonEnum lessonEnum = LessonEnum.valueOf(lessonName);
			//Enumdaki dersin adından dersin idsi bulunuyor. Bulunmuyorsa runtime exception atıyor.
			Long lessonId = lessonRepository.findByLessonName(lessonEnum)
					.orElseThrow(() -> new RuntimeException("no lesson")).getId();
			//
			LessonDetail lessonDetail = new LessonDetail();
			//ders detailden ders idsi, öğrenci numarası verilerek dersin
			lessonDetail.setLessonId(lessonId);
			lessonDetail.setStudentNo(studentNo);
			//dışardan girilen öğrenci adı ve ders adından student id ve
			// lesson id bulunarak bu idler lessonDetail tablosuna kayıt ediliyor.
